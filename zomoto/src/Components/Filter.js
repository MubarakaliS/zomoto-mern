...
const Filter = () => {
  ...
  const mealItem = () => {
    axios({
      method: "POST",
      url: `https://backend-api-rlne.onrender.com/filter`,
  ...
  useEffect(() => {
    const getMealType = sessionStorage.getItem("mealType");
  ...
  return (
    <>
      ...
      <div className="headline mt-3">
        <p style={{ color: "black" }}>{mealType} places in Mumbai</p>
      </div>
      <div className="main-container">
        ...
        {currentMealData.map((item,index) => {
          console.log(item);
          return (
            <>
              <div
                className="flex-parent flex-item2"
                style={{ cursor: "pointer" }}
                key={item._id}
                onClick={() => handleNavigate(item._id)}
              >
                <img src={item.thumb[index]} alt="No image found" />
                <h3
                  style={{
                    marginLeft: "17%",
                    lineHeight: "normal",
